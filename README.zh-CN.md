...
谷歌 CTO 办公室的 Senior Director、Distinguished Engineer——Antonio Gulli——
写了一本《Agentic Design Patterns》，讲 AI Agent 的 21 个设计模式，400 多页，免费放出来。

书是好书。但全英文、纯文字、流程图是静态的。没有技术背景的人，
大概读到第三章就放弃了。
...
- **每个模式都是一张能动的图解**——按「运行」，看数据怎么一步步流过去，
  也可以自己按左右键一步步走
- **先给生活化类比，再上专业定义**——反思 = 作者与编辑；路由 = 医院分诊台；
  工具调用 = 给顾问一部电话和一本通讯录。先用你熟悉的东西建立直觉
- **但它不是「简化版」。** 每个模式都保留了原书的专业术语（中英对照）、
  对应页码、以及背后那篇论文的链接——**你随时能追到源头去核实，
  而不是只能相信我的转述**

最后这条是整个设计的核心。**市面上的入门内容大多是浅的，严谨的内容大多是读不懂的。
这个东西试图两样都不是。**
...
| | 是什么 | 给谁的 |
|---|---|---|
| **`agentic-patterns-lab/`** | 中英双语的交互式网页 | **想搞懂 Agent 的任何人。** 不用写代码、不用安装、不用注册，打开就能看 |
| **`agent-workshop/`** | 一个真能跑的 Python 项目 | **可选。** 等你哪天想自己动手搭一个的时候再来 |

第二部分你完全可以无视，不影响你把这件事搞明白。但如果你去了，两半是连着的：
网页的「实战路径」页写清每一步对应哪个文件、敲什么命令；
每个模式详情页底部也会告诉你「这个模式在实战路径第几步用到」。
